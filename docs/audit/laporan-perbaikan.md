...
# Perubahan pasca-Batch 7

## Pesan otomatis tombol "Chat Sekarang" di Pusat Bantuan (22 September 2026)

Atas permintaan pemilik sistem, tombol "Chat Sekarang" di Pusat Bantuan kini membawa pesan pembuka otomatis ("Halo! Saya sedang butuh bantuan.") lewat parameter `?text=` pada tautan wa.me, memakai kembali `KontakBantuan::tautanWhatsApp()` yang sudah mendukung parameter `$pesan` opsional. Tautan "Hubungi Sub-Bagian Umum" di halaman masuk tidak diubah — pemanggilnya (`two-panel.blade.php`, `footer-publik.blade.php`) tetap tidak mengisi parameter itu sehingga tetap memakai pesan bawaan lama. Berkas yang diubah: `app/Filament/Pages/PusatBantuan.php` (memanggil helper dengan teks pesan, bukan `null`), `app/Support/KontakBantuan.php` (perbarui komentar docblock agar sesuai pemakaian baru), `tests/Feature/PusatBantuanTest.php` (test diperbarui untuk memeriksa pesan baru, bukan lagi tanpa pesan). Verifikasi: seluruh 695 test lulus; struktur tautan dicek lewat tinker — `https://wa.me/6281238096104?text=Halo%21%20Saya%20sedang%20butuh%20bantuan.` untuk Pusat Bantuan, sedangkan halaman masuk tetap `...?text=Halo%2C%20saya%20mengalami%20kendala%20masuk%20ke%20SIMPBI%20dan%20memerlukan%20bantuan.`.

## Perbaikan pasca-Batch 8: validasi NUP unik pada Aset Tetap (23 September 2026)

**Akar masalah.** Field `nup` pada `AsetTetapForm.php` (Buat maupun Ubah) tidak memiliki aturan `unique()` sama sekali, padahal kolomnya murni unik pada dirinya sendiri di basis data (`$table->string('nup', 30)->unique()`, tanpa kolom pasangan). Mengisi NUP yang sudah dipakai aset lain karena itu lolos validasi form dan baru gagal saat disimpan, tampil sebagai `UniqueConstraintViolationException` mentah (galat 500) — bukan pesan yang dapat dipahami pengguna. Pola ini sama dengan A-021 (Batch 2) pada "Barang Baru" Stok Masuk, yang sudah diperbaiki lebih dulu; form Aset Tetap belum ikut ditangani saat itu.

`ImporAsetTetap.php` sendiri **tidak kena masalah yang sama** untuk kasus normal: setiap baris mencari asetnya lewat NUP (`AsetTetap::where('nup', $nup)->first()`) sebelum memutuskan membuat atau memperbarui, dan tiap baris disimpan dalam transaksinya sendiri secara berurutan — sehingga dua baris ber-NUP sama pada satu berkas otomatis menjadi "tambah lalu perbarui", bukan dua kali "tambah". Yang tersisa hanyalah celah kondisi pacu yang sempit: pencarian dan pembuatan bukan satu kesatuan atomik, sehingga dua unggahan yang bersamaan persis dapat sama-sama tidak menemukan barisnya lalu sama-sama mencoba membuat. Jaring pengaman ditambahkan juga di sini, mengikuti prinsip yang sama.

**Berkas yang diubah:**
- `app/Filament/Resources/AsetTetaps/Schemas/AsetTetapForm.php` — field `nup` ditambah `->trim()->unique(ignoreRecord: true)->validationMessages(['unique' => 'NUP sudah dipakai oleh aset lain.'])`.
- `app/Filament/Resources/AsetTetaps/Pages/CreateAsetTetap.php` dan `EditAsetTetap.php` — jaring pengaman di server: `handleRecordCreation()`/`handleRecordUpdate()` ditimpa untuk menangkap `UniqueConstraintViolationException` yang lolos dari aturan form (dua penyimpanan bersamaan persis), diubah menjadi notifikasi yang sama lalu `Halt`, bukan galat basis data mentah.
- `app/Services/Impor/ImporAsetTetap.php` — pembuatan baris baru (bukan pembaruan) dibungkus try/catch serupa; baris yang bentrok ditolak lewat `catatGalat()` dengan pesan yang sama, baris lain pada berkas yang sama tetap diproses.

Tidak ada migration, perubahan alur bisnis, hak akses, atau struktur data. Tidak ada perubahan pada Blade PDF, kelas ekspor Excel, atau template impor Aset Tetap — dikonfirmasi lewat `git diff --stat` (hanya empat berkas di atas) dan pembacaan kode (tidak satu pun menyentuh `DokumenPermintaanService`, `KartuKendaliService`, `PembuatTemplate`, atau berkas `.blade.php`).

**Tes baru** (`tests/Feature/NupUnikAsetTetapTest.php`, 6; ditambah 1 di `SinkronisasiAsetTetapTest.php`): NUP yang sudah dipakai ditolak di form Buat dengan pesan validasi (bukan galat 500); NUP dengan spasi ujung tetap terdeteksi bentrok; NUP baru tetap berhasil dibuat; jaring pengaman server menangani pelanggaran UNIQUE yang lolos validasi form (disimulasikan lewat kait `creating`, pola sama dengan uji A-021); form Ubah tidak menganggap aset bentrok dengan NUP miliknya sendiri (`ignoreRecord`); form Ubah menolak NUP milik aset lain; `ImporAsetTetap` menolak baris yang bentrok akibat kondisi pacu dengan pesan jelas sementara baris lain tetap diproses.

**Hasil pengujian.** Garis dasar sebelum perbaikan: 698 tes lolos (5165 assersi) di SQLite maupun MySQL 8.4.3 sementara. Sesudah perbaikan: 705 tes lolos (5206 assersi) di kedua driver — garis dasar ditambah 7 tes baru, tanpa ada tes lama yang berubah atau gagal. Uji mundur: keempat berkas yang diubah dikembalikan sementara ke versi asli, dan 5 dari 7 tes baru gagal — keempatnya dengan `UniqueConstraintViolationException` mentah persis seperti bukti laporan bug (2 yang tetap lolos menguji perilaku yang memang sudah benar sebelumnya: NUP baru, dan penyuntingan tanpa mengubah NUP). Berkas dipulihkan dan dicocokkan sha256 dengan salinan sesudah perbaikan (4 dari 4 identik).

## Perbaikan pasca-Batch 8: notifikasi tidak tertandai dibaca saat diklik (23 September 2026)

**Akar masalah — (c) navigasi terjadi sebelum permintaan tandai-dibaca selesai diproses.** Baris notifikasi pada `resources/views/livewire/lonceng-notifikasi.blade.php` dirender sebagai `<a href="{{ $tautan }}" wire:click="tandaiDibaca({{ $n->id }})">` — tautan `<a href>` polos yang SEKALIGUS memikul `wire:click`, tanpa penanda `.prevent`. Pada klik, peramban menjalankan navigasi bawaan `href` seketika, sementara permintaan Livewire yang memanggil `tandaiDibaca()` baru selesai diproses belakangan secara asinkron; halaman keburu berpindah sebelum baris `UPDATE ... SET dibaca_at = ...` sempat dikirim/diproses server, sehingga titik merah tetap ada. Method `tandaiDibaca(int $id)` di `app/Livewire/LoncengNotifikasi.php` sendiri sudah benar sejak awal — tersaring kepemilikan lewat `kueri()` (menyaring `user_id`) dan `belumDibaca()` — bukan itu masalahnya.

**Perbaikan.**
- `resources/views/livewire/lonceng-notifikasi.blade.php`: `wire:click="tandaiDibaca(...)"` → `wire:click.prevent="tandaiDibaca(...)"`, sehingga navigasi bawaan `href` tidak lagi berjalan sendiri; `href` tetap dipertahankan (pratinjau tautan, klik-tengah/Ctrl+klik untuk tab baru tetap berfungsi karena keduanya tidak melalui `wire:click`).
- `app/Livewire/LoncengNotifikasi.php`: `tandaiDibaca(int $id)` diperluas — mengambil notifikasinya lebih dulu (lewat `kueri()`, sekaligus penjaga kepemilikan), menandainya dibaca dengan kueri yang sama seperti semula, lalu memanggil `$this->redirect($tautan)` bila notifikasi itu punya tujuan. Penandaan dan pengalihan kini satu permintaan Livewire yang sama, sehingga urutannya terjamin: dibaca dulu, baru berpindah. Badge (`getJumlahBelumDibacaProperty()`) dan daftar ikut diperbarui otomatis oleh siklus render Livewire yang sama, tanpa menunggu `wire:poll` 15 detik berikutnya.
- Tidak menyentuh `sembunyikan()` maupun `tandaiSemuaDibaca()` — kemampuan menyingkirkan notifikasi dan alur "Tandai semua dibaca" tidak diubah sama sekali.

**Tes baru** (`tests/Feature/TandaiDibacaLoncengTest.php`, 3): mengklik notifikasi ber-tautan menandainya dibaca, badge berkurang seketika (tanpa polling), dan komponen mengalihkan ke tujuan yang benar (`assertRedirect`); notifikasi lain milik pengguna yang sama tidak ikut tertandai; notifikasi milik pengguna lain tidak dapat ditandai lewat id yang ditebak.

**Hasil pengujian.** Garis dasar sebelum perbaikan: 705 tes lolos (5206 assersi) di SQLite maupun MySQL 8.4.3 sementara. Sesudah perbaikan: 708 tes lolos (5214 assersi) di kedua driver — garis dasar ditambah 3 tes baru. Uji mundur: kedua berkas dikembalikan sementara ke versi asli; 1 dari 3 tes baru gagal dengan pesan persis `Component did not perform a redirect` — tepat menangkap bug ini (dua tes lain tetap lolos sebab hanya menguji kepemilikan/penyaringan kueri, yang memang sudah benar sebelumnya). Berkas dipulihkan dan dicocokkan sha256 dengan salinan sesudah perbaikan (2 dari 2 identik).

**Penelusuran peramban headless** (server verifikasi terpisah, MySQL 8.4.3 sementara `simpbi_notif_test`, satu notifikasi uji ber-tautan untuk akun Kasubbag): badge menampilkan "1" sebelum diklik; mengklik baris notifikasi memicu satu permintaan `POST .../livewire/update`; dalam 1,5 detik TANPA reload manual, badge hilang dari layar yang sama dan peramban berpindah ke `…/permintaan-barangs?tableAction=detail&tableActionRecord=1&filters[kode_permintaan][value]=PB-NOTIF-UJI` (pop-up Rincian yang benar). Dikonfirmasi langsung di basis data: kolom `dibaca_at` notifikasi itu terisi — bukti bukan sekadar efek tampilan.

## Verifikasi pasca-Batch 8: penerima notifikasi tujuh status dan penanganan galat Mutasi Aset (23 September 2026)

Tugas investigasi murni (tanpa perbaikan kode) atas sembilan jalur yang dicatat "Tidak tercakup" pada `docs/pengujian`: penerima notifikasi tujuh transisi status permintaan barang (NS-12 s.d. NS-18, `NotifikasiService::permintaanBerubah()`) dan dua jalur penanganan galat Mutasi Aset (MA-5, MA-9, `MutasiAsetService`/`CreateBastMutasiAset`). **Catatan lokasi:** NS-12 s.d. NS-18 sebenarnya tercatat pada `docs/pengujian/04-modul-4-dashboard-monitoring.md`, bukan `02-modul-2-permintaan-barang.md` seperti disebut pada instruksi tugas — dikoreksi di sini berdasarkan pembacaan langsung berkasnya.

**Kondisi awal.** Basis data kerja: `wa_aktif='1'`, `wa_alihkan_ke` **terisi** (mode peragaan sedang menyala) — dicatat apa adanya, **tidak diubah** sepanjang tugas ini (diverifikasi ulang sesudah seluruh pengujian, hasilnya identik). Seluruh reproduksi dijalankan pada basis data sementara (`RefreshDatabase`, tidak pernah menyentuh `database.sqlite`) dengan `wa_alihkan_ke` dinyalakan khusus di situ dan `Queue::fake()` mencegah pengiriman sungguhan.

### B. Penerima notifikasi (NS-12 s.d. NS-18)

Ketujuh transisi diuji dengan menjalankan aksinya lewat Livewire (bukan hanya membaca kode), memeriksa **baris in-app dan payload job WhatsApp** (`KirimPesanWhatsApp::notifikasiId`, ditelusuri balik ke baris `notifikasi` bersangkutan — bukan sekadar `assertDispatched` tanpa argumen).

| Kasus | Transisi (aksi) | Penerima seharusnya (kode) | Penerima aktual (diukur) | Status |
|---|---|---|---|---|
| NS-12 | `menunggu_verifikasi` → `menunggu_kasubbag` (Verifikasi Gudang) | Kasubbag Umum | Kasubbag Umum (in-app + WA); Gudang & Tim **tidak** menerima | **Sesuai** |
| NS-13 | `menunggu_kasubbag` → `siap_diproses` (Setujui Kasubbag) | Petugas Gudang | Petugas Gudang (in-app + WA); Kasubbag **tidak** menerima | **Sesuai** |
| NS-14 | `siap_diproses` → `siap_diambil` (Siapkan) | Tim + Ketua Tim pemohon | Tim + Ketua Tim (in-app + WA masing-masing); Gudang **tidak** menerima | **Sesuai** |
| NS-15 | `siap_diambil` → `menunggu_pengesahan` (Konfirmasi, sesuai) | Kasubbag Umum | Kasubbag Umum (in-app + WA); Tim/Ketua **tidak** menerima | **Sesuai** |
| NS-16 | `menunggu_pengesahan` → `selesai` (Sahkan) | Tim + Ketua Tim pemohon | Tim + Ketua Tim (in-app + WA); Kasubbag **tidak** menerima | **Sesuai** |
| NS-17a | Tolak tahap Ketua Tim | Tim + Ketua Tim pemohon, pesan menyertakan alasan | Tim + Ketua Tim (in-app + WA); pesan memuat teks alasan persis | **Sesuai** |
| NS-17b | Tolak tahap Kasubbag | Tim + Ketua Tim pemohon, pesan menyertakan alasan | Tim + Ketua Tim (in-app + WA); pesan memuat teks alasan persis; Kasubbag **tidak** menerima | **Sesuai** |
| NS-18 | Konfirmasi, barang tidak sesuai & tidak dapat diatasi | Kasubbag + Petugas Gudang + Tim/Ketua Tim pemohon | Keempatnya menerima (in-app + WA) | **Sesuai** |

**Tidak ditemukan bug** — seluruh penerima persis sesuai kode `NotifikasiService::permintaanBerubah()` (dibaca dan diuji), termasuk pola pemanggilan `$record->refresh()` sebelum tiap panggilan `permintaanBerubah()` di `PermintaanBarangResource.php` yang mencegah kelas bug "closure menangkap status lama" yang pernah ditemukan sebelumnya.

### C. Penanganan galat Mutasi Aset (MA-5, MA-9)

**MA-9** (`periksaPembuatan()`: aset tidak ditemukan). Direproduksi dengan `aset_id` fiktif (999999) lewat `fillForm()` langsung (pola "muatan dimodifikasi", bukan lewat pilihan Select biasa). **Temuan: tidak sampai memicu `periksaPembuatan()` sama sekali** — kolom `aset_id` pada `BastMutasiAsetForm` adalah Select ber-`relationship()`, dan Filament/Livewire memvalidasinya di sisi server sebagai bagian aturan formulir SEBELUM `handleRecordCreation()` dipanggil, menghasilkan pesan spesifik berbahasa Indonesia ("aset (NUP — Nama) yang dipilih tidak valid.") — bukan galat 500 mentah, bahkan tertangkap lebih awal daripada yang diasumsikan `docs/pengujian`. `grep` mengonfirmasi `periksaPembuatan()` hanya dipanggil dari satu titik, yang sama-sama dilindungi validasi Select ini — baris "Aset tidak ditemukan." adalah pengaman kedua yang sehat (jaga-jaga bila dipanggil dari jalur lain di masa depan), bukan kode mati yang berbahaya. **Status: TERKONFIRMASI AMAN.**

**MA-5** (galat teknis dilempar ulang, bukan disamarkan sebagai penolakan bisnis). Ditelusuri: `DokumenBastService::buat()` (pembentuk dokumen BAST) dipanggil dari `CreateBastMutasiAset::afterCreate()` — **lifecycle hook terpisah**, di LUAR `try/catch(\RuntimeException)` milik `handleRecordCreation()`. Direproduksi dengan mengganti `DokumenBastService` lewat container binding agar `buat()` melempar `\RuntimeException` murni (galat teknis simulasi, bukan aturan bisnis). **Hasil: exception tersebut menjalar tanpa tertangkap** sampai ke pemanggil (`$lemparan !== null`), dibuktikan di dalam `tests/Feature/VerifikasiNotifikasiMutasiAsetTest.php::test_ma5_galat_teknis_tetap_menjalar_sesuai_desain_bukan_disamarkan` — persis sesuai maksud yang dinyatakan komentar kode sendiri ("Penolakan aturan bisnis tampil apa adanya; galat teknis dilempar ulang"). Dalam produksi (APP_DEBUG=false) ini bermuara pada halaman galat generik Laravel (bukan jejak tumpukan mentah), bukan notifikasi ramah dalam aplikasi — sebuah **pilihan desain yang disengaja** supaya galat teknis nyata tetap terlihat oleh pengembang/log, bukan diam-diam tersamar sebagai penolakan bisnis yang membingungkan. **Status: TERKONFIRMASI SESUAI DESAIN — bukan bug**, karena perilakunya persis seperti yang dimaksudkan kode; mengubahnya (mis. menjadikannya notifikasi ramah) justru akan bertentangan dengan niat desain yang sudah tertulis eksplisit.

**Catatan investigasi tambahan (bukan bug, tidak memerlukan tindakan):** pengujian lewat `Livewire::test()` terhadap satu pemicu galat teknis lain (kejadian di dalam `DB::transaction` milik `handleRecordCreation()`, disimulasikan lewat kait model `creating`) menunjukkan perkakas uji Livewire dapat "menyerap" exception dalam siklus pembaruan komponennya sendiri sehingga tidak selalu tampak sebagai exception PHP yang tertangkap test — kerumitan test-harness, bukan perilaku aplikasi; tidak dipakai sebagai dasar kesimpulan, digantikan bukti yang lebih andal (container-binding) di atas.

### Kesimpulan bagian D

**Tidak ada bug yang ditemukan** pada kesembilan jalur (NS-12 s.d. NS-18, MA-5, MA-9) — seluruhnya **TERKONFIRMASI AMAN** atau **TERKONFIRMASI SESUAI DESAIN**. Bagian D (perbaikan) dilewati sepenuhnya sesuai instruksi tugas; **tidak ada satu pun berkas kode aplikasi yang diubah**. Satu berkas tes ringan ditambahkan sebagai bukti verifikasi permanen: `tests/Feature/VerifikasiNotifikasiMutasiAsetTest.php` (10 tes). Suite penuh: **718 lolos (5328 assersi)** di SQLite maupun MySQL 8.4.3 sementara (garis dasar 708 + 10 tes baru, tidak ada tes lama yang berubah). Basis data kerja dan mode peragaannya tidak berubah dari kondisi awal (diverifikasi ulang).

## Pembalikan urutan alur Mutasi Aset (23 September 2026)

Eksekusi atas `docs/audit/rencana-pembalikan-alur-mutasi-aset.md` (Fase 1, investigasi murni sebelumnya) berdasarkan keputusan pemilik yang menjawab bagian G rencana itu, ditambah keputusan baru yang mengganti sebagian rencana B/C. Skill: tidak ada skill proyek yang relevan (sama seperti batch sebelumnya, isi `.claude/skills` seluruhnya desain/animasi front-end).

**Lingkup.** (1) Membalik urutan BAST: Buat → `menunggu_konfirmasi` → **Konfirmasi Penerimaan** (Ketua Tim tujuan; aset berpindah di sini) → `menunggu_pengesahan` → **Sahkan** (Kasubbag; finalisasi dokumen, langkah terakhir) → `selesai_administratif`. (2) Pembuatan BAST jadi pop-up (`CreateAction`), bukan halaman terpisah. (3) Aksi Sahkan/Konfirmasi jadi dialog rincian yang tuntas sekali klik — **ketik NIP dihapus sepenuhnya** dari ketiga aksi BAST (keputusan baru, menggantikan rencana lama "dialog + ketik NIP"). (4) Kolom Pihak Penyerah/Penerima dihapus dari UI, diisi otomatis dari nama Ketua Tim. (5) Penjaga tanda tangan dua lapis sebelum BAST dapat dibuat. (6) Gaya tombol Sahkan/Konfirmasi disamakan dengan Permintaan Barang. **Batch F (wajib Tim Kerja saat Buat Aset Tetap) TIDAK termasuk cakupan ini** — `app/Filament/Resources/AsetTetaps/`, `app/Services/Impor/ImporAsetTetap.php`, dan `PembuatTemplate` tidak disentuh sama sekali, dikonfirmasi lewat `git status` di akhir tugas.

### Pagar keamanan

| Butir | Keterangan |
|---|---|
| Git | Hanya perintah baca (`status`, `diff`, `log`). Tidak ada `add`/`commit`. |
| Cadangan | Setiap berkas disalin **sebelum** disentuh ke `E:\Projek Skripsi\cadangan-audit\pembalikan-mutasi-aset\asli\` (path relatif utuh), dengan `manifest.md` berisi sha256 sebelum **dan** sesudah setiap berkas, dicocokkan ulang lewat uji mundur (lihat bawah). Dua berkas uji ternyata ikut terdampak namun terlewat dari daftar awal rencana (`PenempatanAsetFormTest.php`, `SinkronisasiAsetTetapTest.php`, ditemukan lewat kegagalan suite penuh) — dicadangkan begitu ditemukan, sebelum disunting. |
| Basis data kerja | Tidak disentuh — dikonfirmasi lewat mtime `database/database.sqlite` (21:11, sebelum tugas ini dimulai, tidak berubah sepanjang sesi) dan `git status` (kosong untuk berkas itu). |
| MySQL sementara | Sempat diblokir cacat lingkungan pra-ada (instalasi `mysql-8.4.3-winx64` lama tidak memiliki direktori `lib/plugin` sama sekali, sehingga `mysqld.exe` gagal start memuat komponen bawaan `component_reference_cache`, errno 126) — **sudah diperbaiki 24 September 2026**: instalasi lengkap baru diunduh resmi dari `dev.mysql.com` dan dipasang terpisah di `E:\laragon\laragon\bin\mysql\mysql-8.4.3-winx64-utuh\` (folder baru, instalasi lama dan `mysql-8.0.30-winx64`/layanan Laragon port 3306 tidak disentuh sama sekali — dikonfirmasi identik tiga lapis, proses/port/`mysqladmin ping`, sebelum dan sesudah, PID 3306 tidak berubah). Instans sementara dari instalasi baru ini (datadir scratch, port 3399) **berhasil dijalankan dan menjalankan suite penuh: 727 lolos (5404 assersi) — identik persis dengan hasil SQLite**, termasuk seluruh test yang memakai `lockForUpdate()`/`kunciDanPastikanStatus()` pada Mutasi Aset (tidak ada kegagalan yang hanya muncul di MySQL). Instans dimatikan bersih lewat `mysqladmin shutdown`. Instalasi baru ini tersedia untuk dipakai batch-batch berikutnya. |
| WhatsApp | Tidak ada pesan sungguhan dikirim; seluruh reproduksi memakai `RefreshDatabase` pada basis data sementara, `Storage::fake()`. |
| PDF beku | Dibuktikan lewat perbandingan byte-demi-byte (lihat bagian tersendiri di bawah), bukan sekadar diasumsikan dari investigasi Fase 1. `DokumenBastService.php` dan `resources/views/pdf/bast-mutasi.blade.php` **tidak disentuh sama sekali** — dikonfirmasi `git status` kosong untuk kedua berkas itu. |

### Ringkasan perubahan per berkas

| Berkas | Perubahan |
|---|---|
| `app/Services/MutasiAsetService.php` | `pesanAsetTidakDapatDimutasi()`: status yang memblokir menjadi `menunggu_konfirmasi` (dari `menunggu_pengesahan`), pesan disesuaikan. `periksaPembuatan()`: tambah parameter `?int $timTujuanId` dan pemanggilan penjaga tanda tangan baru (E). `sahkan()`: dipangkas jadi finalisasi murni — tidak lagi mengunci/memeriksa/memindahkan aset atau menulis riwayat; syarat status **input** tetap `menunggu_pengesahan` (tidak berubah), keluaran kini `selesai_administratif` (dari `menunggu_konfirmasi`). `konfirmasi()`: menyerap logika pemindahan aset + pemeriksaan "usang" yang dipindah dari `sahkan()` lama; syarat status input tetap `menunggu_konfirmasi` (tidak berubah), keluaran kini `menunggu_pengesahan` (dari `selesai_administratif`). Method baru `pesanTandaTanganBelumLengkap(?Tim, ?Tim): ?string` (E), memakai `Tim::ketuaTim()` (sumber A-012) dan `TandaTangan::terdaftar()`. `kunciDanPastikanStatus()` tidak disentuh. |
| `app/Services/NotifikasiService.php` | **Tidak diubah sama sekali** (G.1 — lihat di bawah). |
| `app/Filament/Resources/BastMutasiAsets/Pages/CreateBastMutasiAset.php` | **Dihapus.** Logikanya (`mutateFormDataBeforeCreate`, `handleRecordCreation` dengan retry nomor bentrok, `afterCreate`) dipindah ke `CreateAction` pada `ListBastMutasiAsets`. |
| `app/Filament/Resources/BastMutasiAsets/BastMutasiAsetResource.php` | `getPages()`: entri `'create'` dihapus (B) — tidak ada lagi halaman Buat terpisah. |
| `app/Filament/Resources/BastMutasiAsets/Pages/ListBastMutasiAsets.php` | `CreateAction` dilengkapi `mutateFormDataUsing()` (isi `nomor_bast`, `dibuat_oleh_id`, `status='menunggu_konfirmasi'`, dan `pihak_penyerah`/`pihak_penerima` otomatis dari nama Ketua Tim — D, G.3), `using()` (method baru `handleCreation()`: transaksi + `periksaPembuatan()` + retry bentrok nomor, dipindah dari halaman lama), `after()` (bentuk dokumen draf + notifikasi, dipindah dari `afterCreate()` lama). |
| `app/Filament/Resources/BastMutasiAsets/Schemas/BastMutasiAsetForm.php` | Dua `TextInput` (`pihak_penyerah`, `pihak_penerima`) dihapus (D). Tambah `->live()` pada `tim_tujuan_id` dan satu `Placeholder` peringatan (`penjagaTandaTangan`) yang tampil bila salah satu Ketua Tim belum bertanda tangan (E, lapis UX) — method statis baru `pesanTandaTangan(Get $get)` dipakai bersama oleh Placeholder ini dan penjaga di `ListBastMutasiAsets`. |
| `app/Filament/Resources/BastMutasiAsets/Tables/BastMutasiAsetsTable.php` | Aksi `sahkan`/`konfirmasi`: `->requiresConfirmation()` + `modalDescription()` polos diganti `->schema([View::make('filament.partials.detail-bast')...])` (dialog rincian wajib tampil, C) + `->button()` (gaya tombol disamakan Permintaan Barang). `konfirmasi` warna diubah `info` → `success` (menyamai `aksiKonfirmasi()` Permintaan Barang persis). Teks `modalDescription()` Sahkan diperbaiki (tidak lagi mengklaim memindahkan aset). `->visible()` pada kedua aksi **tidak diubah** (syarat status input tidak berubah). |
| `resources/views/filament/partials/detail-bast.blade.php` | **Baru.** Isi dialog rincian: status, nomor BAST, aset, Tim Asal/Tujuan, alasan mutasi, pelaksana tiap tahap — meniru gaya `detail-permintaan.blade.php` (kotak status berwarna, `dl`/`dt`/`dd`). |
| `database/seeders/BastMutasiAsetSeeder.php` | Memanggil `konfirmasi()` lalu `sahkan()` (urutan baru; sebelumnya hanya `sahkan()`), `status` awal eksplisit `menunggu_konfirmasi`, `pihak_penyerah`/`pihak_penerima` dari nama Ketua Tim (bukan nama petugas gudang), tambah penjaga tanda tangan (melewati seeding bila salah satu Ketua Tim belum bertanda tangan, bukan menerbitkan contoh yang mustahil lewat panel sungguhan). |
| `docs/pengujian/03-modul-3-mutasi-aset.md` | Tabel B dan C (jalur MA-6 s.d. MA-20) diperbarui mengikuti urutan baru; dua baris baru ditambahkan (MA-8b penjaga TTD, MA-21/MA-22 dialog+pengisian otomatis). Ditandai eksplisit **bukan audit ulang menyeluruh** — sesi pengujian berikutnya disarankan memetakan ulang dari nol. |
| `docs/audit/rencana-pembalikan-alur-mutasi-aset.md` | Bagian G ditandai terjawab sesuai keputusan pemilik (G.1, G.3, G.5 dijawab; F.3/F.5 dikonfirmasi di luar cakupan). |

**Sembilan berkas uji BAST inti ditulis ulang** (`MutasiAsetPenempatanTest`, `StatusBastDanAsetAktifTest`, `PenomoranBastTest`, `BastTandaTanganTersimpanTest`, `DokumenBastTest`, `NotifikasiMutasiAsetTest`, `RiwayatPenempatanAsetTest`, `VerifikasiNotifikasiMutasiAsetTest` — `BastMutasiAsetAksesTest` tidak disentuh, tidak terkait order/UI) sesuai pemetaan rencana bagian B, ditambah **empat berkas yang tidak tercantum di rencana Fase 1** namun ditemukan lewat kegagalan suite penuh: `PenempatanAsetFormTest.php`, `SinkronisasiAsetTetapTest.php` (keduanya memanggil `sahkan()` langsung dan mengasumsikan ia memindahkan aset), `DokumenDiDiskPrivatTest.php`, dan skrip debug pra-sesi `DebugMa5d.php` (keduanya memakai halaman `CreateBastMutasiAset` yang dihapus). Satu berkas uji baru: `tests/Feature/PopUpBastTanpaKetikNipTest.php` (9 tes) — dialog rincian wajib tampil tanpa kolom ketik NIP, aksi tuntas sekali klik, penjaga tanda tangan (peringatan reaktif + penolakan server + jalur berhasil), dan pengisian otomatis `pihak_penyerah`/`pihak_penerima`.

### G.1 — Notifikasi: teks tidak diubah, hanya titik pemanggilan

Sesuai keputusan pemilik, `NotifikasiService::bastBerubah()` **tidak disentuh sama sekali** — dikonfirmasi `git diff` kosong untuk berkas itu. Penerima per status tidak pernah bergantung pada urutan (status `menunggu_pengesahan` selalu berarti "Kasubbag perlu bertindak", `menunggu_konfirmasi` selalu berarti "Ketua Tim tujuan perlu bertindak", terlepas kapan status itu tercapai), sehingga memindahkan TITIK PEMANGGILAN cukup dengan sendirinya mengikuti urutan baru:
- Saat BAST dibuat: status awal kini `menunggu_konfirmasi` (bukan `menunggu_pengesahan`) → cabang yang sama di `bastBerubah()` otomatis memberi tahu Ketua Tim tujuan, bukan Kasubbag.
- Saat Konfirmasi Penerimaan: status keluaran kini `menunggu_pengesahan` (bukan `selesai_administratif`) → otomatis memberi tahu Kasubbag.
- Saat Sahkan: status keluaran tetap `selesai_administratif` (langkah akhir tidak berubah) → tetap memberi tahu Kasubbag + Petugas Gudang.

Dua test di `NotifikasiMutasiAsetTest.php` yang namanya mengasumsikan urutan lama (`test_bast_baru_memberitahu_kasubbag_saja`, `test_bast_disahkan_memberitahu_ketua_tim_tujuan_saja`) ditulis ulang namanya (`test_bast_baru_memberitahu_ketua_tim_tujuan_saja`, `test_bast_dikonfirmasi_memberitahu_kasubbag_saja`) dengan skenario status yang ditukar — assersi penerima-per-status itu sendiri tidak berubah kodenya.

### Perbandingan keluaran dokumen (bukti PDF beku, bukan asumsi)

Dibuktikan dengan menjalankan skenario BAST lengkap lewat **dua implementasi service dalam proses PHP terpisah** (menghindari artefak penomoran objek gambar DomPDF yang kumulatif dalam satu proses, ditemukan pada percobaan pertama — lihat catatan di bawah), pada data yang identik persis (tim, aset, Ketua Tim bertanda tangan sama, token QR dipatri manual bukan diacak, waktu dibekukan lewat `Carbon::setTestNow()`):

- **Alur lama** (tiruan persis method `sahkan()`/`konfirmasi()` asli dari cadangan, dipanggil sahkan dulu baru konfirmasi): `keluaran-awal/bast-alur-lama.pdf`, 71.732 bita.
- **Alur baru** (kode kerja apa adanya, konfirmasi dulu baru sahkan): `keluaran-akhir/bast-alur-baru.pdf`, 71.732 bita — **ukuran identik**.

`cmp -l` menghitung **62 dari 71.732 bita berbeda**, seluruhnya berada di dua tempat: (1) field `/CreationDate` dan `/ModDate` — beda 10 detik, persis selisih waktu nyata antara dua pemanggilan `php artisan test` yang terpisah (DomPDF menulis timestamp sistem sungguhan untuk metadata ini, tidak dipengaruhi `Carbon::setTestNow()` yang hanya mengendalikan `now()` aplikasi); (2) `/ID[...]` pada trailer PDF, turunan hash dari isi berkas termasuk `/CreationDate` — berubah sebagai akibat (1), bukan perubahan independen. `/Size`, `/Root`, `startxref`, dan seluruh isi lain (termasuk jumlah dan nomor objek gambar) **identik**. **Kesimpulan: tata letak dokumen BAST terbukti tidak berubah struktur sama sekali** — persis sesuai temuan investigasi Fase 1 (A.6): hanya WAKTU pengisian tiap blok tanda tangan yang berubah (blok yang dulu terisi saat `sahkan()` kini terisi saat `konfirmasi()`, dan sebaliknya), bukan tata letaknya.

*Catatan metodologi:* percobaan pertama menjalankan kedua skenario berurutan dalam **satu** proses PHP menghasilkan ukuran berkas berbeda (71.732 vs 72.333 bita, dua gambar tambahan pada berkas kedua) — ternyata artefak DomPDF yang menomori objek gambar secara kumulatif lintas pemanggilan `Pdf::loadView()->output()` dalam satu proses yang sama, bukan perbedaan sungguhan pada kode. Kedua skenario dipisah jadi dua method test yang dijalankan lewat dua pemanggilan `php artisan test --filter` terpisah untuk menghilangkan artefak itu; berkas skrip (`tests/Feature/SementaraPerbandinganPdfBastTest.php`) dihapus setelah bukti diambil, sesuai catatan di kepala berkasnya sendiri — bukan bagian regresi tetap.

### Hasil pengujian

| Tahap | SQLite (`:memory:`) | MySQL 8.4.3 sementara |
|---|---|---|
| Garis dasar (sebelum tugas ini) | **718 lolos (5328 assersi)**, 596 dtk | Tidak dijalankan (cacat instalasi lokal, sejak diperbaiki 24 September 2026) |
| Sesudah Fase 1–3 (kode + tes ditulis ulang, tes baru ditambah) | **727 lolos (5404 assersi)**, 251 dtk | **727 lolos (5404 assersi)**, 285 dtk — 24 September 2026, instalasi `mysql-8.4.3-winx64-utuh` |

Garis dasar 718 + 9 tes baru (`PopUpBastTanpaKetikNipTest`) = 727 — **tidak ada tes lama yang gagal atau hilang**, seluruh berkas yang ditulis ulang tetap menjaga jumlah metode tesnya (hanya isi/nama yang berubah, bukan jumlahnya), kecuali `NotifikasiMutasiAsetTest.php` (dua metode berganti nama, jumlah tetap 6). **Kedua driver menghasilkan angka identik persis** — tidak ada kegagalan yang hanya muncul di MySQL, termasuk pada jalur `lockForUpdate()`/`kunciDanPastikanStatus()` Mutasi Aset yang baru benar-benar terkunci di MySQL (SQLite tidak mengenal kunci baris).

**Uji mundur** (Fase 3.3): `app/Services/MutasiAsetService.php` dikembalikan sementara ke versi asli (sha256 dicocokkan dengan `manifest.md`), lalu berkas uji inti yang paling bergantung pada logika ini dijalankan (`MutasiAsetPenempatanTest`, `StatusBastDanAsetAktifTest`, `PopUpBastTanpaKetikNipTest`, `BastTandaTanganTersimpanTest` — 39 tes gabungan). Hasil: **27 gagal, 12 lolos** — kegagalan mencakup baik assersi status yang tertukar (mis. `sahkan()` versi lama masih mengeluarkan `menunggu_konfirmasi`, bukan `selesai_administratif` yang diharapkan tes baru) maupun galat "Call to undefined method `pesanTandaTanganBelumLengkap()`" (membuktikan penjaga tanda tangan E memang baru, bukan sudah ada sebelumnya). Berkas dipulihkan ke versi yang sudah diperbaiki dan dicocokkan ulang sha256 (identik dengan `manifest.md`); seluruh 39 tes lolos kembali sesudah dipulihkan.

### Penelusuran peramban — TIDAK TUNTAS (keterbatasan lingkungan, dicatat apa adanya)

Server terisolasi berhasil disiapkan (basis data SQLite sementara di scratchpad, storage salinan, `php artisan migrate:fresh --seed` berhasil penuh termasuk `BastMutasiAsetSeeder` dengan urutan barunya, `php artisan serve` merespons HTTP 200 pada `/admin/login`) — dibuktikan lewat `curl`, bukan diasumsikan. **Namun tidak ada perkakas otomasi peramban yang tersedia di lingkungan ini**: tidak ada koneksi MCP peramban, tidak ada instalasi Playwright/Puppeteer lokal, `chromium-cli` tidak ditemukan. Skill `browser-automation` dipanggil tetapi tidak menyediakan mekanisme penggerak nyata pada lingkungan ini. Server sementara sudah dihentikan sesudahnya (proses `php artisan serve` beserta proses `php -S` di baliknya, keduanya dipastikan lewat `netstat`/command line sebelum dihentikan — bukan layanan MySQL/PHP milik pemilik). **Sebagai gantinya**, siklus penuh satu BAST (Buat → Konfirmasi → Sahkan → tombol Unduh muncul) diverifikasi lewat `Livewire::test()` di level PHPUnit — yang menjalankan komponen Livewire/Filament sungguhan (mount, render HTML nyata, panggil aksi, cek status akhir), bukan sekadar memanggil method PHP secara langsung — pada `StatusBastDanAsetAktifTest::test_alur_normal_konfirmasi_lalu_sahkan_tetap_berhasil` dan seluruh `PopUpBastTanpaKetikNipTest`. Ini bukti kuat bahwa komponennya bekerja, tetapi **bukan pengganti sungguhan** bagi tangkapan layar/interaksi klik nyata yang diminta tugas ini.

## Perbaikan pasca-Fase 0-6: akses Buat BAST (24 September 2026)

Skill: tidak ada skill proyek yang relevan (sama seperti batch sebelumnya). Cadangan: `E:\Projek Skripsi\cadangan-audit\akses-buat-bast\asli\` + `manifest.md` (sha256 sebelum/sesudah). MySQL sementara memakai biner yang sudah diperbaiki `mysql-8.4.3-winx64-utuh`.

### Akar masalah — **(b) bug otorisasi SERVER sungguhan, risiko TINGGI** (bukan sekadar bug tampilan)

Dibuktikan lewat reproduksi nyata (`mountAction('create')->setActionData([...])->callMountedAction()` — bukan `callAction()`, yang punya pre-cek `assertActionVisible()` bawaan sendiri dan karena itu tidak membuktikan apa pun soal perilaku runtime sungguhan): akun **Ketua Tim** dan akun **Tim** BERHASIL menyimpan BAST ke database (`BastMutasiAset::count()` 0 → 1, tanpa exception apa pun).

**Penyebab persis.** `BastMutasiAsetResource::canCreate()` (`return auth()->user()?->role === 'petugas_gudang';`) **tidak berubah sama sekali** sejak sebelum Fase 0-6 (dikonfirmasi `diff` byte-demi-byte dengan cadangan) — tetapi method ini **tidak pernah dipanggil di mana pun** pada kode hasil Fase 0-6. Sebelum Fase 0-6, `CreateBastMutasiAset extends CreateRecord` (halaman Filament) menegakkannya lewat `mount()` bawaan Filament: `abort_unless(static::getResource()::canCreate(), 403);` (`vendor/filament/filament/src/Resources/Pages/CreateRecord.php:75`). Fase 0-6 mengganti halaman itu dengan `CreateAction::make()` polos di `ListBastMutasiAsets::getHeaderActions()` — **`Filament\Actions\CreateAction` TIDAK memiliki pemeriksaan otorisasi bawaan apa pun** (dikonfirmasi baca langsung `vendor/filament/actions/src/CreateAction.php`: `setUp()` hanya mengatur label, judul modal, dan aksi "buat lagi" — tidak ada `canCreate()`/`authorize()` sama sekali). Akibatnya pemeriksaan peran yang dulu otomatis ditegakkan Filament hilang total, di kedua lapis (tampilan **dan** server) sekaligus.

**A2.4 — Sahkan/Konfirmasi: TIDAK kena celah yang sama, dikonfirmasi lewat investigasi, bukan asumsi.** Kedua aksi ini punya `->visible(fn () => ... auth()->user()?->role === ...)`. Dibuktikan lewat pembacaan kode Filament (`Actions\Concerns\InteractsWithActions::mountAction()` memeriksa `$action->isDisabled()` — yang menguji `isHidden()`, yang membaca closure `->visible()` yang sama — SEBELUM aksi apa pun dijalankan) **dan** reproduksi langsung (`mountAction(TestAction::make(...)->table($bast))->callMountedAction()` untuk peran Tim/Ketua Tim lain/Kasubbag-pada-Konfirmasi/Admin — status BAST **tidak pernah berubah** untuk satu pun percobaan). Kesimpulan: `->visible()` Filament **bukan sekadar kosmetik UI** — ia genuinely ditegakkan di titik mount runtime, sehingga Sahkan/Konfirmasi sudah aman sejak awal; tidak ada perbaikan tambahan diperlukan di sana.

### Berkas yang diubah

| Berkas | Perubahan |
|---|---|
| `app/Filament/Resources/BastMutasiAsets/Pages/ListBastMutasiAsets.php` | `CreateAction`: tambah `->visible(fn () => BastMutasiAsetResource::canCreate())` (lapis UI, pola sama dengan Sahkan/Konfirmasi). `handleCreation()`: tambah `abort_unless(BastMutasiAsetResource::canCreate(), 403);` sebagai baris pertama — **jaring pengaman utama**, memulihkan persis mekanisme `CreateRecord::mount()` yang hilang, ditegakkan terlepas dari tampilan tombol. |

Tidak ada berkas lain yang disentuh untuk Bagian A — `BastMutasiAsetResource::canCreate()`/`canAccess()` dan seluruh aksi Sahkan/Konfirmasi tidak diubah sama sekali.

**Tes baru** (`tests/Feature/AksesBuatBastTest.php`, 10 tes): Ketua Tim/Tim/Kasubbag ditolak membuat BAST lewat pop-up (mount mentah, bukan `callAction()`); `handleCreation()` melempar `HttpException` 403 lewat Reflection langsung (bukti lapis server independen dari Livewire); Petugas Gudang tetap berhasil (kontrol regresi); tombol tersembunyi bagi Ketua Tim/Tim, tampil bagi Gudang; Sahkan/Konfirmasi tetap menolak peran yang salah (regresi A2.4, termasuk kasus Admin yang malah ditolak lebih awal lagi oleh `canAccess()` halaman); Sahkan/Konfirmasi tetap berhasil untuk peran yang benar (kontrol); `canCreate()` tetap membatasi ke Petugas Gudang.

## Perbaikan Batch F: Tim Kerja wajib saat Buat Aset Tetap (24 September 2026)

Sesuai `docs/audit/rencana-pembalikan-alur-mutasi-aset.md` bagian F (temuan investigasi sebelumnya): field `tim_penempatan_id` pada form **Buat** Aset Tetap kini `required()`, placeholder "Belum ditempatkan" hanya muncul di **Ubah**. Logika kunci-bersyarat F-002 di Ubah **tidak disentuh**. `ImporAsetTetap.php`/`PembuatTemplate` **tidak disentuh** — kolom Tim Penempatan pada template impor tetap opsional sesuai keputusan sebelumnya.

### Berkas yang diubah

| Berkas | Perubahan |
|---|---|
| `app/Filament/Resources/AsetTetaps/Schemas/AsetTetapForm.php` | `tim_penempatan_id`: `->required(fn (string $operation) => $operation === 'create')` dan `->placeholder(fn (string $operation) => $operation === 'create' ? null : 'Belum ditempatkan')` — pola `$operation` yang sama persis dengan presedan `UserForm.php:73`. `->disabled()`/`->dehydrated()` (kunci-bersyarat F-002 di Ubah) **tidak diubah**. |
| `tests/Feature/RiwayatPenempatanAsetTest.php` | 4 tes yang menguji "buat aset tanpa tim lewat form" ditulis ulang: 1 jadi assersi penolakan (`test_form_buat_menolak_submit_tanpa_tim_penempatan`), 3 lainnya (skenario F-002 lewat Ubah) diubah memakai `buatAset()` langsung (bukan form Buat) sebagai fixture data lama, isi pengujiannya tidak berubah. |
| `tests/Feature/PenempatanAsetFormTest.php` | Tambah 3 tes: form Buat menolak tanpa tim; placeholder hanya di Ubah; regresi F-002 eksplisit. 1 tes lama (`test_form_buat_berhasil_dengan_kolom_sinkronisasi_kosong`, soal F-003) diberi tim pada fixture-nya (tidak terkait tim, sekadar supaya tidak tertolak aturan berbeda). |
| `tests/Feature/NupUnikAsetTetapTest.php` | **Ditemukan lewat kegagalan suite penuh, tidak tercantum di rencana awal**: `isianAset()` diberi `tim_penempatan_id` (aset baru per NUP unik, tidak terkait pengujian keunikan NUP itu sendiri). |

**Tes baru/diubah:** 4 tes ditulis ulang + 3 tes baru di `PenempatanAsetFormTest.php` + penyesuaian fixture di `NupUnikAsetTetapTest.php` (2 tes, tanpa menambah/mengurangi jumlah).

### Hasil pengujian (Bagian A + B digabung)

| Tahap | SQLite (`:memory:`) | MySQL 8.4.3 sementara (`-utuh`) |
|---|---|---|
| Garis dasar (sebelum tugas ini) | **727 lolos (5404 assersi)** — dikonfirmasi ulang bersih (lihat catatan flake di bawah) | Tidak dijalankan ulang (sudah diverifikasi identik pada batch MySQL sebelumnya) |
| Sesudah Bagian A + B | **740 lolos (5450 assersi)**, 261 dtk | **740 lolos (5450 assersi)** — **identik persis** |

727 + 10 (`AksesBuatBastTest`) + 3 (`PenempatanAsetFormTest` baru) = 740. **Tidak ada tes lama yang gagal atau hilang di kedua driver.**

*Catatan flake:* percobaan garis dasar pertama (dijalankan bersamaan dengan skrip reproduksi celah akses di proses PHP terpisah) menunjukkan 2 kegagalan pada `KonfirmasiAkunTimTest`/`PersetujuanTidakMelebihiDimintaTest` (modul Permintaan Barang, tidak tersentuh batch ini) — dijalankan ulang sendirian (tanpa proses lain berjalan bersamaan) dan bersih 727/5404, mengonfirmasi kegagalan pertama adalah artefak kontensi sumber daya dari beberapa proses `php artisan test` berjalan bersamaan, bukan regresi sungguhan.

**Uji mundur.** Kedua berkas inti dikembalikan sementara ke versi asli (sha256 dicocokkan dengan `manifest.md` sebelum dan sesudah):
- `ListBastMutasiAsets.php` → asli: **5 dari 10 tes `AksesBuatBastTest` gagal** (Ketua Tim/Tim/Kasubbag berhasil membuat BAST; `handleCreation()` tidak melempar 403; tombol tidak tersembunyi) — persis menangkap bug ini.
- `AsetTetapForm.php` → asli: **3 dari 36 tes gabungan (`PenempatanAsetFormTest`+`RiwayatPenempatanAsetTest`) gagal** (form Buat menerima submit tanpa tim; placeholder masih muncul di Buat) — persis menangkap kelalaian Batch F.

Kedua berkas dipulihkan ke versi yang sudah diperbaiki; sha256 dicocokkan ulang (identik), seluruh tes lolos kembali sesudah dipulihkan.
